...
read one attribute chain off it. `_meta` is faked as a PLAIN DICT because that is what the
SDK hands over — `RequestParamsMeta` is a TypedDict, so it has no `model_dump`. A fake with
one is how this path came to be inert in production while every test here passed.
"""
import asyncio
import types
...
def test_the_call_id_is_read_out_of_request_meta():
    assert mcp._tool_use_id(_ctx({"claudecode/toolUseId": "toolu_1"})) == "toolu_1"
...
def test_every_missing_piece_of_the_meta_chain_is_simply_no_id():
    ...
    an SDK that shapes the context differently, a stamp that is not the string it is
    supposed to be — each means "unknown caller", which resolves the way it resolves
    today. None of them is an error the caller should see, because the call itself is
    perfectly fine."""
    class Exploding:
        def get(self, _k):
            raise RuntimeError("not a mapping after all")

    for ctx in (None,
                _ctx(None),
                _ctx({}),
                _ctx({"other": "field"}),
                _ctx({"claudecode/toolUseId": 17}),
                _ctx({"claudecode/toolUseId": None}),
                _ctx(Exploding()),
                _ctx(object()),
...
@pytest.mark.parametrize("call", [
    lambda: mcp.exec_tool("1+1", ctx=_ctx({"claudecode/toolUseId": "toolu_9"})),
    lambda: mcp.wait_tool(1, ctx=_ctx({"claudecode/toolUseId": "toolu_9"})),
    lambda: mcp.interrupt_tool(ctx=_ctx({"claudecode/toolUseId": "toolu_9"})),
    lambda: mcp.restart_tool(ctx=_ctx({"claudecode/toolUseId": "toolu_9"})),
])
