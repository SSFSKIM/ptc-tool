...
| `wait` | `cell_id: int`, `session?`, `timeout_s: int = 300`, `max_output_chars: int = 12000`, `since: int = -1`, `until?: str` | Return output produced after offset `since` in `cells/<id>.log` (default `-1` = the offset this adapter last served, else 0), plus the cell's state from the terminal record. Returns `next_offset` — a caller-held cursor, so retries are idempotent and two waiters cannot consume each other's output. Completion (status, result repr, error, duration, images) comes from `cells/<id>.json`, so a **fresh adapter** can settle a cell it never started. May yield again. `until` is a Python regex that makes the wait an **event**: it returns `status: running` with `matched` as soon as NEW output first matches, rather than at settle — a long budget plus `until=` turns the completion notification into a trigger. First match wins (`matched` is clipped to 512 chars — it names the event, it does not carry the data); completion supersedes the scan, re-checked immediately before the match is returned; an uncompilable pattern is a tool error. The window holds at least the most recent bounded read, so a longer match is not guaranteed — and when it drops output it read past, `window_start` names the byte the returned output begins at, which the text render and `--json` both report. |
